...
class KeyedCircularPhaseSwitchEnv(BaseEnv):
    ...
    def __init__(self, *args, robot_uids="panda", robot_init_qpos_noise=0.0, orientation=None, task_anchor=None, keyed=True, geometry_scale=1.0, **kwargs):
        self.robot_init_qpos_noise = robot_init_qpos_noise
        self.causal_delta = np.zeros(3, dtype=np.float64)
        self.socket_yaw = 0.0
        self.keyed = bool(keyed)
        # Geometry scale: proportional lateral scaling of the peg shaft, bore and
        # socket annulus, so two instances of the SAME relation family can differ
        # in physical size while keeping the same generator semantics. Axial
        # constants (heights, gate z, phase schedule) are unchanged. The circular
        # gate clearance is kept constant so the peg always fits the gate.
        self.geometry_scale = float(geometry_scale)
        self.shaft_radius = SHAFT_RADIUS * self.geometry_scale
        self.bore_inner_radius = BORE_INNER_RADIUS * self.geometry_scale
        self.socket_outer_radius = SOCKET_OUTER_RADIUS * self.geometry_scale
        self.circular_gate_inner_radius = self.shaft_radius + GATE_CLEARANCE
